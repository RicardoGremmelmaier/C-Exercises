    ...
    m[x_queijo][y_queijo] = dist;
     ...
     while (!encheu){
        ...
        for (i = 0; i < h; i++)
            for (j = 0; j < w; j++){
                if (m[i][j] == dist){
                    if (i + 1 <= h && m[i+1][j] == CAMINHO){
                        m[i+1][j] = dist + 1;
                        preencheu = 1;
                    }

                    if (i - 1 >= 0 && m[i-1][j] == CAMINHO){
                        m[i-1][j] = dist + 1;
                        preencheu = 1;
                    }

                    if (j + 1 <= w && m[i][j+1] == CAMINHO){
                        m[i][j+1] = dist + 1;
                        preencheu = 1;
                    }

                    if (j - 1 >= 0 && m[i][j-1] == CAMINHO){
                        m[i][j-1] = dist + 1;
                        preencheu = 1;
                    }
                }
...
int calculaCaminho (int** m, int w, int h, int x_rato, int y_rato, int** caminho_x, int** caminho_y){
    int distAtual, caminhoTot = 0,
        i = 0, j = 0,
        *vetorX,*vetorY;

    distAtual = m[x_rato][y_rato];

    vetorX = (int*) malloc (sizeof(int)* (distAtual+1));
    vetorY = (int*) malloc (sizeof(int)* (distAtual+1));


    vetorX[i] = x_rato;
    vetorY[j] = y_rato;
    caminhoTot += distAtual;

    /* Enquanto a distancia nao for 0, procuro um local a proximo do
     * rato em que a distancia seja distAtual - 1 e atualizo ela*/
