/*
 * Aluno: Ricardo Marthus Gremmelmaier RA: 2401266
*/
...
int main()
{
    ...
    /*
     * Abre um arquivo txt e o coloca em uma matriz, colocando
     * assim essa matriz na funcao de jogo
    */
...
void jogo(int **matriz, int nl, int nc, int iter)
{
    ...
    /*
     * Cria uma copia da matriz, anterior as modificacoes no loop, da geracao
     * inicial das celulas, para nao modificar a matriz durante a geracao atual
     * das celulas
    */
    ...
    for(k=0; k<iter; k++)
    {
        for(i=1;i<nl-1;i++) //percorre linhas ignorando as bordas
        {
            ...
            {
                /*
                 * Caso a celula esteja viva e sua vizinha for maior que 3
                 * ou menor que dois, a celula morre
                 * Caso a celula esteja morta e a vizinha seja exatamente 3
                 * a celula renasce
                */
        ...
        /*
         * Aqui recriamos uma copia da matriz, mudada apos a geracao ter atualizado
         * completamente para repetir o processo de iteracao
        */
...
int** alocaMatriz(int nl, int nc){
    ...
    //Aloco dinamicamente memoria para o ponteiro de ponteiro de acordo com a qnt de linhas
    matriz = (int**) malloc (sizeof(int*) * nl);

    //Para cada linha aloco memoria dinamicamente o suficiente para o tam de colunas
    for (i = 0; i < nl; i++)
...
void destroiMatriz(int **m, int nl){
    ...
    //Desaloco inicialmente onde os dados sao armazenados
    for (i = 0; i < nl; i++)
        free(m[i]);

    //Depois desaloco o ponteiro de ponteiro inicial
    free(m);
}
